...
global lOptionsChangeFolderInDialogText := "Normally, you change the folder in a dialog box using the Power menu hotkey [~1~] or [~2~] and choose ""Select the folder for this dialog box"".`n`nIf you turn this option ON, the main QAP hotkeys [~3~] or [~4~] will support folder navigation in dialog boxes. But you must use this feature only in ""file dialog boxes"" (Open, Save As, etc.). If you use it in other types of dialog box, for example an Options dialog box, trying to change folder could modify and save values in the dialog box without any notification."
